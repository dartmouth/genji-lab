...
const SimpleSearchBar: React.FC = () => {
  ...
  const limit = useSelector(
    (state: RootState) => state.searchResults.settings.limit
  );
  const searchTypes = useSelector(
    (state: RootState) => state.searchResults.settings.searchTypes
  );
  const sortBy = useSelector(
    (state: RootState) => state.searchResults.settings.sortBy
  );
  const sortOrder = useSelector(
    (state: RootState) => state.searchResults.settings.sortOrder
  );
  ...
  const parseSearchQuery = (query: string): ParsedQueryItem[] => {
    ...
    const parseTokens = (
      tokens: string[],
      startIndex = 0
    ): { result: ParsedQueryItem[]; endIndex: number } => {
