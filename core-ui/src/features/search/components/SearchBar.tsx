...
const AnnotationSearchBar: React.FC<SearchBarProps> = ({
  ...
  const parseSearchQuery = (query: string): ParsedSearchTerm[] => {
    ...
    const parseTokens = (
      tokens: string[],
      startIndex = 0
    ): { result: ParsedSearchTerm[]; endIndex: number } => {
      ...
      while (i < tokens.length) {
        ...
        if (token === ")") {
          ...
          const operator =
            result.length === 0 ? null : getOperatorBefore(tokens, i) || "AND";
  ...
  const parseSimpleQuery = (query: string): ParsedSearchTerm[] => {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (token.toUpperCase() === "AND" || token.toUpperCase() === "OR") {
        ...
        const operator =
          prevToken &&
          (prevToken.toUpperCase() === "AND" ||
            prevToken.toUpperCase() === "OR")
    ...
    items.forEach((item, index) => {
      ...
      if (item.type === "term") {
        elements.push(
          <span
            key={`term-${index}`}
            className="px-3 py-1 bg-white border border-blue-300 text-blue-800 text-sm rounded"
          >
            "{item.term}"
          </span>
        );
        ...
        elements.push(
          <div
            key={`group-${index}`}
            className="flex items-center gap-1 px-2 py-1 bg-purple-100 border border-purple-300 rounded-lg"
          >
      ...
      {searchQuery.parsedQuery.length > 0 && (
        <div className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
          ...
          <div className="text-xs text-blue-600 mt-2">
            ...
            <code>(python OR java) AND "machine learning"</code>. Quote phrases
            with spaces.
          </div>
        </div>
      )}
                ...
                <div className="p-2">
                  ...
                  <label className="flex items-center px-2 py-1 hover:bg-gray-50 cursor-pointer rounded">
                    <input
                      ...
                      disabled={
                        searchQuery.searchTypes.length === 1 &&
                        searchQuery.searchTypes.includes("annotations")
                      }
                    />
        ...
        <div className="border-t border-gray-200 pt-4 space-y-4">
          ...
          <div>
            ...
            <div className="text-xs text-gray-500 mt-1">
              Press Enter, comma, or semicolon to add a tag. Backspace to remove
              the last tag.
            </div>
          </div>
