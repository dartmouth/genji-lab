...
const ReplyForm: React.FC<ReplyFormProps> = ({
  ...
  const handleInsertLink = () => {
    ...
    const newText =
      replyText.substring(0, selectionStart) +
      markdownLink +
  ...
  const handleSave = () => {
    ...
    const classId =
      activeClassroomValue && !(isOptedOut === "true")
        ...
        : undefined;
    dispatch(
      slice.thunks.saveAnnotation({ annotation: payload, classroomId: classId })
    );
    setReplyText("");
    ...
    >
      ...
      <div style={{ position: "relative" }}>
        <textarea
          ...
          placeholder={
            motivation === "replying"
              ? "Write your reply..."
              : "Why should this content be flagged for review?"
          }
          value={replyText}
      ...
      >
        <button
          ...
          style={{
            opacity: !replyText.trim() || isOverLimit ? 0.5 : 1,
            cursor:
              !replyText.trim() || isOverLimit ? "not-allowed" : "pointer",
          }}
        >
          Submit
        </button>
