...
const ManageDocuments: React.FC = () => {
  ...
  const [importWordFormData, setImportWordFormData] =
    useState<ImportWordFormData>({
      document_collection_id: undefined,
      title: "",
      description: "",
      file: null,
  ...
  const [importWordSubmitted, setImportWordSubmitted] =
    useState<boolean>(false);
  const [importWordLoading, setImportWordLoading] = useState<boolean>(false);
  const [selectedImportWordCollection, setSelectedImportWordCollection] =
    useState<string>("");
  ...
  const [selectedDeleteCollection, setSelectedDeleteCollection] =
    useState<string>("");
  const [selectedDocuments, setSelectedDocuments] = useState<number[]>([]);
  const [deleteAllInCollection, setDeleteAllInCollection] =
    useState<boolean>(false);
  const [isDeleting, setIsDeleting] = useState<boolean>(false);
  const [documentsWithStats, setDocumentsWithStats] = useState<
    Array<{
      id: number;
      title: string;
      description: string;
  ...
  const [selectedContentDeleteCollection, setSelectedContentDeleteCollection] =
    useState<string>("");
  const [selectedContentDeleteDocument, setSelectedContentDeleteDocument] =
    useState<string>("");
  ...
  const [renameSelectedCollection, setRenameSelectedCollection] =
    useState<string>("");
  const [renameSelectedDocument, setRenameSelectedDocument] =
    useState<string>("");
  const [renameNewName, setRenameNewName] = useState<string>("");
  const [isRenaming, setIsRenaming] = useState<boolean>(false);
  const [isLoadingRenameDocuments, setIsLoadingRenameDocuments] =
    useState<boolean>(false);
  const [renameDocuments, setRenameDocuments] = useState<
    Array<{
      id: number;
      title: string;
      description: string;
  ...
  const [
    updateDescriptionSelectedCollection,
    setUpdateDescriptionSelectedCollection,
  ] = useState<string>("");
  const [
  ...
  const [updateDescriptionNewDescription, setUpdateDescriptionNewDescription] =
    useState<string>("");
  const [isUpdatingDescription, setIsUpdatingDescription] =
    useState<boolean>(false);
  const [
    isLoadingUpdateDescriptionDocuments,
    setIsLoadingUpdateDescriptionDocuments,
  ] = useState<boolean>(false);
  const [updateDescriptionDocuments, setUpdateDescriptionDocuments] = useState<
    Array<{
      id: number;
      title: string;
      description: string;
    ...
    if (importWordFormData.title.trim() && collectionId) {
      ...
      const nameExists = documentsInCollection.some(
        (doc) =>
          ...
          importWordFormData.title.trim().toLowerCase()
      );
  ...
  const handleImportWordTitleChange = (
      ...
      setTimeout(() => {
        ...
        setImportWordFormData((currentData) => {
          if (
            ...
            const documentsInCollection = documents.filter(
              (doc) =>
                doc.document_collection_id ===
                currentData.document_collection_id
            );

            const nameExists = documentsInCollection.some(
              (doc) => doc.title.toLowerCase() === newTitle.trim().toLowerCase()
            );
  ...
  const handleImportWordDescriptionChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    setImportWordFormData((prevData) => ({
      ...prevData,
  ...
  const handleImportWordFileSelect = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
  ...
  const handleImportWordSubmit = async (e: React.FormEvent) => {
    ...
    if (
      !importWordFormData.document_collection_id ||
      !importWordFormData.title.trim() ||
      !importWordFormData.file
    ) {
      setImportWordError(
        "Please select a collection, enter a title, and select a file"
      );
      return;
    }
  ...
  const initiateContentDelete = () => {
    ...
    const selectedDoc = documents.find(
      (d) => d.id === parseInt(selectedContentDeleteDocument)
    );
    const selectedCollection = documentCollections.find(
      (c) => c.id === parseInt(selectedContentDeleteCollection)
    );
...
The document itself will remain but will be empty. This action cannot be undone.`;
  ...
  const handleConfirmContentDelete = async () => {
    ...
    try {
      ...
      const response = await api.delete(
        `/elements/document/${documentId}/all-elements?force=true`
      );
      console.log(`Delete status is ${response.status}`);
  ...
  const handleRenameDocumentSelect = (event: any) => {
    ...
    if (documentId) {
      const selectedDoc = renameDocuments.find(
  ...
  const handleRenameDocument = async () => {
    ...
    const selectedDocument = renameDocuments.find(
      (d) => d.id === parseInt(renameSelectedDocument)
    );
    if (!selectedDocument) {
    ...
    const nameExists = renameDocuments.some(
      (d) =>
    ...
    try {
      ...
      if (renameSelectedCollection) {
        const result = await dispatch(
          fetchDocumentsByCollection(parseInt(renameSelectedCollection))
        ).unwrap();
        setRenameDocuments(result.documents);
      }
  ...
  const handleUpdateDescriptionDocumentSelect = (event: any) => {
    ...
    if (documentId) {
      const selectedDoc = updateDescriptionDocuments.find(
        (d) => d.id === parseInt(documentId)
      );
  ...
  const handleUpdateDescriptionNewDescriptionChange = (
  ...
  const handleUpdateDocumentDescription = async () => {
    ...
    const selectedDocument = updateDescriptionDocuments.find(
      (d) => d.id === parseInt(updateDescriptionSelectedDocument)
    );
    if (!selectedDocument) {
      showNotification("Selected document not found", "error");
      return;
    ...
    if (
      updateDescriptionNewDescription.trim() === selectedDocument.description
    ) {
      showNotification(
    ...
    try {
      ...
      if (updateDescriptionSelectedCollection) {
        const result = await dispatch(
          fetchDocumentsByCollection(
            parseInt(updateDescriptionSelectedCollection)
          )
        ).unwrap();
        setUpdateDescriptionDocuments(result.documents);

        // Update the description in the form to reflect the change
        const updatedDoc = result.documents.find(
          (d) => d.id === parseInt(updateDescriptionSelectedDocument)
        );
    ...
    <Box>
      ...
      <Box sx={{ display: "flex", height: "auto" }}>
        ...
        <SubTabPanel value={activeSubTab} index={0}>
          ...
          <div>
            ...
            <ul>
              ...
              <li>
                <strong>Delete Document Content:</strong> Remove all content
                from a document while keeping the document itself
              </li>
              <li>
                <strong>Rename:</strong> Change document titles
              </li>
              <li>
                <strong>Update Description:</strong> Modify document
                descriptions
              </li>
            </ul>
          </div>
        ...
        <SubTabPanel value={activeSubTab} index={1}>
          ...
          <div>
            <p>
              Create a new document and import Word document (.docx) content.
            </p>
          </div>
          <StyledForm onSubmit={handleImportWordSubmit}>
            ...
            <button
              type="submit"
              disabled={importWordLoading || !!importWordTitleError}
            >
          ...
          {importWordSubmitted && importWordSuccess && importedDocumentData && (
            <div className="submitted-data">
              ...
              <p>
                ...
                {importedDocumentData.collection_name}
              </p>
              <p>
                  ...
                  <FormControlLabel
                    control={
                      <Checkbox
                        ...
                        onChange={(e) =>
                          handleDeleteAllChange(e.target.checked)
                        }
                        color="error"
                      />
                    }
                    ...
                    >
                      {documentsWithStats.map((document) => {
                        ...
                        if (document.scholarly_annotation_count > 0) {
                          annotationText.push(
                            `${
                              document.scholarly_annotation_count
                            } scholarly annotation${
                              document.scholarly_annotation_count !== 1
                                ? "s"
                                : ""
                            }`
                          );
        ...
        <SubTabPanel value={activeSubTab} index={3}>
          ...
          <div>
            <p>
              Select a document to delete all of its content. The document
              itself will remain but all content and annotations will be
              permanently deleted.
            </p>
          ...
          <StyledForm>
            ...
            {selectedContentDeleteCollection &&
              documentsInSelectedCollection.length === 0 && (
                <Typography
                  variant="body2"
                  color="text.secondary"
                  sx={{ marginTop: 2 }}
        ...
        <SubTabPanel value={activeSubTab} index={4}>
          ...
          <StyledForm>
            ...
            <FormControl fullWidth sx={{ maxWidth: "400px" }}>
              <InputLabel id="rename-document-select-label">
                Document
              </InputLabel>
              <Select
        ...
        <SubTabPanel value={activeSubTab} index={5}>
          ...
          <StyledForm>
            ...
            <Button
              ...
              disabled={
                isUpdatingDescription || !isUpdateDescriptionFormValid()
              }
              sx={{ marginTop: 2 }}
            >
              ...
              <div>
                ...
                >
                  {deleteConfirmDialog.documentsToDelete.map((doc, index) => {
                    const annotationText = [];
                    if (
                      doc.scholarly_annotation_count &&
                      doc.scholarly_annotation_count > 0
                    ) {
                      annotationText.push(
                        `${
                          doc.scholarly_annotation_count
                    ...
                    if (doc.comment_count && doc.comment_count > 0) {
                      annotationText.push(
                        `${doc.comment_count} comment${
                          doc.comment_count !== 1 ? "s" : ""
                        }`
                      );
                    }
                    const annotationSuffix =
