...
import {
  ...
  clearCreateStatus,
  selectCurrentClassroomMemberIds,
...
const isClassroomEnded = (classroom: Classroom): boolean => {
...
const isJoinLinkExpired = (classroom: Classroom): boolean => {
...
const getJoinLinkExpirationDate = (classroom: Classroom): string => {
...
const ManageClassrooms: React.FC = () => {
  ...
  const [createdClassroomId, setCreatedClassroomId] = useState<number | null>(
    null
  );
  ...
  const [newlyAddedStudentId, setNewlyAddedStudentId] = useState<number | null>(
    null
  );
  ...
  const classroomsLoading =
    useAppSelector(selectClassroomsStatus) === "loading";
  ...
  useEffect(() => {
    if (activeSubTab === 2 && userCanManage) {
      // Classroom Admin tab
  ...
  const handleStudentAdded = (user: User) => {
    ...
    setSuccessMessage(
      `${user.first_name} ${user.last_name} has been added to the classroom!`
    );
  ...
  const handleConfirmRemove = async () => {
    ...
    try {
      ...
      // Show success message
      setSuccessMessage(
        `${studentToRemove.name} has been removed from the classroom.`
      );
  ...
  const existingMemberIds = useAppSelector(selectCurrentClassroomMemberIds);
      ...
      <Box sx={{ display: "flex", flexGrow: 1 }}>
        ...
        <Box sx={{ borderRight: 1, borderColor: "divider", minWidth: "200px" }}>
          ...
          >
            ...
            {userCanManage && (
              <Tab label="Create Classroom" {...a11yPropsSubTab(1)} />
            )}
            {userCanManage && (
              <Tab label="Classroom Admin" {...a11yPropsSubTab(2)} />
            )}
          </Tabs>
        </Box>
        ...
        <Box sx={{ flexGrow: 1 }}>
              ...
              <Box>
                ...
                <Box sx={{ mb: 3 }}>
                  ...
                  <Typography variant="body1" sx={{ mb: 1 }}>
                    • <strong>Create Classroom:</strong> Set up new classrooms
                    with names and descriptions
                  </Typography>
                  <Typography variant="body1" sx={{ mb: 1 }}>
                    • <strong>Classroom Admin:</strong> View and manage
                    classrooms
                  </Typography>
                </Box>
              </Box>
          ...
          {userCanManage && (
            <SubTabPanel value={activeSubTab} index={1}>
              ...
              <Typography variant="body1" sx={{ mb: 3 }}>
                Create a new classroom to organize students and course
                materials.
              </Typography>
              ...
              <Box component="form" sx={{ maxWidth: 500 }}>
                ...
                {createStatus === "succeeded" && createdClassroomId && (
                  ...
                  >
                    ...
                    <Typography variant="body2" sx={{ mb: 2 }}>
                      Share this link with students so they can join your
                      classroom.
                      <strong> Note:</strong> Join links expire 2 weeks after
                      ...
                      InputProps={{
                        ...
                        endAdornment: (
                          <InputAdornment position="end">
                            <IconButton
                              onClick={() =>
                                handleCopyJoinLink(createdClassroomId)
                              }
                              edge="end"
                ...
                <CreateButton
                  ...
                  disabled={
                    (!isCreateFormValid || createStatus === "loading") &&
                    createStatus !== "succeeded"
                  }
                  startIcon={
              ...
              <Box sx={{ mb: 3, maxWidth: 400 }}>
                <FormControl fullWidth>
                  ...
                  <Select
                    ...
                    onChange={(e) =>
                      handleClassroomSelection(e.target.value as string)
                    }
              ...
              {selectedClassroomId && currentClassroom && (
                <>
                  <Typography variant="h6" component="h3" gutterBottom>
                    ...
                    {currentClassroom.start_date
                      ? formatDateOnly(currentClassroom.start_date)
                      : "N/A"}
                    -
                    {currentClassroom.end_date
                      ? formatDateOnly(currentClassroom.end_date)
                      : "N/A"}
                    )
                  </Typography>
                  ...
                  >
                    Instructor:{" "}
                    {(() => {
                      const instructor = currentClassroom.members?.find(
                        (m) => m.id === currentClassroom.created_by_id
                      );
                      return instructor
                        ? formatMemberName(instructor)
                        : "Unknown";
                    })()}
                  </Typography>
                  ...
                  <Box
                    sx={{
                      ...
                      p: 2,
                      border: "1px solid",
                      borderColor: isJoinLinkExpired(currentClassroom)
                        ? "warning.main"
                        : "primary.main",
                      borderRadius: 2,
                      backgroundColor: isJoinLinkExpired(currentClassroom)
                        ? "warning.50"
                        : "primary.50",
                    }}
                  >
                    <Typography
                      ...
                      sx={{
                        mb: 1,
                        color: isJoinLinkExpired(currentClassroom)
                          ? "warning.dark"
                          : "primary.dark",
                      }}
                    >
                      <Link sx={{ verticalAlign: "middle", mr: 0.5 }} />{" "}
                      Classroom Join Link
                    </Typography>
                    ...
                    >
                      {isJoinLinkExpired(currentClassroom) ? (
                        <span style={{ color: "#f57c00" }}>
                          <strong>⚠️ Expired:</strong> This join link expired on{" "}
                          {getJoinLinkExpirationDate(currentClassroom)}
                        </span>
                      ) : (
                        <span>
                          <strong>Expires:</strong>{" "}
                          {getJoinLinkExpirationDate(currentClassroom)} (2 weeks
                          after start date)
                        </span>
                      )}
                    </Typography>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      💡{" "}
                      <em>
                        Join links expire automatically 2 weeks after start
                        date, but you can always add students using the "Add
                        Student" button below.
                      </em>
                    </Typography>
                    ...
                    {copyFeedback && (
                      <Alert
                        severity={
                          copyFeedback.includes("Failed") ? "error" : "success"
                        }
                        sx={{ mt: 1 }}
                      >
                        {copyFeedback}
                  ...
                  <Box sx={{ mb: 3 }}>
                    {isClassroomEnded(currentClassroom) ? (
                      <Alert severity="info" sx={{ mb: 2 }}>
                        Students can no longer be added because the class ended
                        on{" "}
                        {currentClassroom.end_date
                          ? formatDateOnly(currentClassroom.end_date)
                          : "N/A"}
                  ...
                  currentClassroom.members.length > 0 ? (
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {currentClassroom.members
                            .filter(
                              (member) =>
                                member.id !== currentClassroom.created_by_id
                            )
                            .map((member) => (
                              <TableRow
                                key={member.id}
