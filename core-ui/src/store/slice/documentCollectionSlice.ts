...
type MetadataValue = string | string[] | Base64Image;
    ...
    try {
      ...
      if (!(response.status === 200)) {
        return rejectWithValue(
          `Failed to fetch document collections: ${response.statusText}`
        );
      }
  ...
  async (newCollection: DocumentCollectionCreate, { rejectWithValue }) => {
    try {
      ...
      if (!(response.status === 201)) {
        return rejectWithValue(
          `Failed to create document collection: ${response.statusText}`
        );
      }
...
export const updateDocumentCollection = createAsyncThunk(
  ...
  async (
    { id, updates }: { id: number; updates: DocumentCollectionUpdate },
    { rejectWithValue }
  ) => {
    try {
      ...
      if (!(response.status === 200)) {
        return rejectWithValue(
          `Failed to update document collection: ${response.statusText}`
        );
      }
...
const documentCollectionSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        fetchDocumentCollections.fulfilled,
        (state, action: PayloadAction<DocumentCollection[]>) => {
      ...
      .addCase(
        createDocumentCollection.fulfilled,
        (state, action: PayloadAction<DocumentCollection>) => {
          state.status = "succeeded";
      ...
      .addCase(
        updateDocumentCollection.fulfilled,
        (state, action: PayloadAction<DocumentCollection>) => {
          state.status = "succeeded";
...
// Export selectors
export const selectAllDocumentCollections = (state: RootState) =>
  state.documentCollections.collections;
export const selectDocumentCollectionsStatus = (state: RootState) =>
  state.documentCollections.status;
export const selectDocumentCollectionsError = (state: RootState) =>
  state.documentCollections.error;
