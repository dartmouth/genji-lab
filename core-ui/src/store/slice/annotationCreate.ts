...
const createAnnotationSlice = createSlice({
  ...
  reducers: {
    ...
    completeSelection: (state) => {
      ...
      state.target.selectedText = sortedSegments.reduce(
        (combinedText, segment, index) => {
...
export const selectAnnotationCreate = (state: RootState) =>
  state.createAnnotation;

// Selector for the motivation
export const selectMotivation = (state: RootState) =>
  state.createAnnotation.motivation;

export const selectSelectedText = (state: RootState) =>
  state.createAnnotation.target.selectedText;
...
export const selectNewAnnotationContent = (state: RootState) =>
  state.createAnnotation.content;
...
export const selectSegmentForParagraph = (
  state: RootState,
  paragraphId: string
) =>
  state.createAnnotation.target.segments.find(
    (segment) => segment.sourceURI === paragraphId
  );
