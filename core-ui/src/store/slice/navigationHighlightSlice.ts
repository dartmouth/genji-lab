...
const navigationHighlightSlice = createSlice({
  ...
  reducers: {
    ...
    addNavigationHighlight: (
      ...
      const normalizedURI = elementURI.startsWith("/")
        ? elementURI.substring(1)
        : elementURI;
    ...
    removeNavigationHighlight: (
      state,
      action: PayloadAction<{ elementURI: string }>
    ) => {
    ...
    clearNavigationSession: (
      ...
      Object.keys(state.activeHighlights).forEach((uri) => {
        if (
          state.activeHighlights[uri].sessionId === action.payload.sessionId
        ) {
...
export const selectIsElementHighlighted =
  (elementURI: string) => (state: RootState) => {
    const normalizedURI = elementURI.startsWith("/")
      ? elementURI.substring(1)
      : elementURI;
    const originalURI = elementURI.startsWith("/")
      ? elementURI
      : `/${elementURI}`;

    return (
      elementURI in state.navigationHighlight.activeHighlights ||
      normalizedURI in state.navigationHighlight.activeHighlights ||
      originalURI in state.navigationHighlight.activeHighlights
    );
  };

export const selectHighlightType =
  (elementURI: string) => (state: RootState) => {
    const normalizedURI = elementURI.startsWith("/")
      ? elementURI.substring(1)
      : elementURI;
    ...
    return (
      state.navigationHighlight.activeHighlights[elementURI]?.type ||
      state.navigationHighlight.activeHighlights[normalizedURI]?.type ||
      state.navigationHighlight.activeHighlights[originalURI]?.type ||
      null
    );
  };
