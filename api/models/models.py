...
class User(Base):
    ...
    annotations = relationship(
        "Annotation", foreign_keys="Annotation.creator_id", back_populates="creator"
    )
    owned_annotations = relationship(
        "Annotation", foreign_keys="Annotation.owner_id", back_populates="owner"
    )
    ...
    created_groups = relationship(
        "Group", foreign_keys="Group.created_by_id", back_populates="created_by"
    )
    created_shares = relationship(
        "ObjectSharing",
        foreign_keys="ObjectSharing.created_by_id",
        back_populates="created_by",
    )
...
class Group(Base):
    ...
    start_date = Column(
        Date, nullable=True
    )  # TODO: Make not nullable after existing groups are updated
    end_date = Column(
        Date, nullable=True
    )  # TODO: Make not nullable after existing groups are updated
...
class DocumentCollection(Base):
    ...
    modified_by = relationship(
        "User", foreign_keys=[modified_by_id], back_populates="modified_collections"
    )
    owner = relationship(
        "User", foreign_keys=[owner_id], back_populates="owned_collections"
    )
...
class Document(Base):
    ...
    document_collection_id = Column(
        Integer, ForeignKey(f"{'app'}.document_collections.id")
    )
    owner_id = Column(Integer, ForeignKey(f"{'app'}.users.id"))
    title = Column(String(255))
...
class Annotation(Base):
    ...
    document_collection = relationship(
        "DocumentCollection", back_populates="annotations"
    )
    document = relationship("Document", back_populates="annotations")
    document_element = relationship("DocumentElement", back_populates="annotations")
    creator = relationship(
        "User", foreign_keys=[creator_id], back_populates="annotations"
    )
    owner = relationship(
...
class CASConfiguration(Base):
    ...
    )  # e.g., "https://login.dartmouth.edu/cas"
    ...
    # Display settings
    display_name = Column(
        String(100), default="CAS Login"
    )  # What to show on login button
...
Index(
    "idx_object_sharing_shared_with",
    ObjectSharing.shared_with_id,
    ObjectSharing.shared_with_type,
)
...
Index(
    "idx_document_collections_hierarchy",
    DocumentCollection.hierarchy,
    postgresql_using="gin",
)
Index(
    "idx_document_collections_metadata",
    DocumentCollection.collection_metadata,
    postgresql_using="gin",
)
Index(
    "idx_document_elements_hierarchy", DocumentElement.hierarchy, postgresql_using="gin"
)
