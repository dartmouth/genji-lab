...
class TestAnnotationServiceHelpers:
    ...
    def test_dump_targets_flat_list(self, annotation_service):
        ...
        targets = [
            ...
            ObjectTarget(type="ObjectTarget", source="image/1"),
        ]
...
class TestAnnotationServiceCreate:
    ...
    def test_create_annotation_success(self, annotation_service, db_session, test_user):
        ...
        result = annotation_service.create(
            db=db_session, annotation=annotation_data, user=test_user, classroom_id=None
        )
    ...
    def test_create_annotation_generates_body_id(
        self, annotation_service, db_session, test_user
    ):
        ...
        annotation_data = AnnotationCreate(
            ...
            generator="test-app",
            body=Body(
                type="TextualBody", value="Test", format="text/plain", language="en"
            ),
            target=[TextTarget(type="TextTarget", source="doc/1", selector=None)],
        )
    ...
    def test_create_annotation_persists_to_database(
        self, annotation_service, db_session, test_user, AnnotationModel
    ):
        ...
        db_annotation = (
            db_session.query(AnnotationModel)
            .filter(AnnotationModel.id == result.id)
            .first()
        )
    ...
    def test_create_annotation_with_classroom(
        self, annotation_service, db_session, test_user, test_classroom
    ):
...
class TestAnnotationServiceGetById:
    ...
    def test_get_by_id_returns_annotation(
        self, annotation_service, db_session, test_annotation
    ):
        """Should retrieve annotation by ID."""
        result = annotation_service.get_by_id(
            db=db_session, annotation_id=test_annotation.id, classroom_id=None
        )
    ...
    def test_get_by_id_not_found_raises_404(self, annotation_service, db_session):
        """Should raise 404 when annotation not found."""
        with pytest.raises(HTTPException) as exc_info:
            annotation_service.get_by_id(
                db=db_session, annotation_id=9999, classroom_id=None
            )

        assert exc_info.value.status_code == 404

    def test_get_by_id_with_classroom_filter(
        self, annotation_service, db_session, test_annotation_with_classroom
    ):
...
class TestAnnotationServiceList:
    ...
    def test_list_applies_motivation_filter(
        self, annotation_service, db_session, multiple_test_annotations
    ):
        ...
        assert all(ann.motivation == "scholarly" for ann in result)
    ...
    def test_list_applies_motivation_filter(
        ...
        """Should filter by motivation."""
        result = annotation_service.list(
            db=db_session, classroom_id=None, motivation="commenting"
        )
    ...
    def test_list_applies_document_element_filter(
        ...
        result = annotation_service.list(
            db=db_session, classroom_id=None, document_element_id=1
        )
    ...
    def test_list_applies_pagination_skip(
        ...
        all_results = annotation_service.list(
            db=db_session, classroom_id=None, skip=0, limit=100
        )

        skipped_results = annotation_service.list(
            db=db_session, classroom_id=None, skip=1, limit=100
        )
    ...
    def test_list_applies_pagination_limit(
        ...
        result = annotation_service.list(
            db=db_session, classroom_id=None, skip=0, limit=2
        )
    ...
    def test_list_default_pagination(
        ...
        result = annotation_service.list(db=db_session, classroom_id=None)
...
class TestAnnotationServiceUpdate:
    ...
    def test_update_body_success(self, annotation_service, db_session, test_annotation):
    ...
    def test_update_persists_to_database(
        self, annotation_service, db_session, test_annotation, AnnotationModel
    ):
        ...
        db_annotation = (
            db_session.query(AnnotationModel)
            .filter(AnnotationModel.id == test_annotation.id)
            .first()
        )

        assert db_annotation.body["value"] == "Persisted update"

    def test_update_not_found_raises_404(self, annotation_service, db_session):
        """Should raise 404 when annotation not found."""
        payload = AnnotationPatch(body="Test")

        with pytest.raises(HTTPException) as exc_info:
            annotation_service.update(
                db=db_session, annotation_id=9999, payload=payload, classroom_id=None
            )
...
class TestAnnotationServiceDelete:
    ...
    def test_delete_success(
        ...
        annotation_service.delete(
            db=db_session, annotation_id=annotation_id, classroom_id=None
        )
    ...
    def test_delete_not_found_raises_404(self, annotation_service, db_session):
        """Should raise 404 when annotation not found."""
        with pytest.raises(HTTPException) as exc_info:
            annotation_service.delete(
                db=db_session, annotation_id=9999, classroom_id=None
    ...
    def test_delete_with_classroom_filter(
        ...
        annotation_service.delete(
            db=db_session, annotation_id=annotation_id, classroom_id=classroom_id
        )
...
class TestAnnotationServiceAddTarget:
    ...
    def test_add_target_success(
        self, annotation_service, db_session, test_annotation, test_user
    ):
    ...
    def test_add_target_generates_id(
        ...
        payload = AnnotationAddTarget(
            target=TextTarget(type="TextTarget", source="doc/new", selector=None)
        )
    ...
    def test_add_target_not_found_raises_404(
        ...
        with pytest.raises(HTTPException) as exc_info:
            annotation_service.add_target(
                db=db_session, annotation_id=9999, payload=payload, user=test_user
            )
...
class TestAnnotationServiceRemoveTarget:
    ...
    def test_remove_target_annotation_not_found_raises_404(
        ...
        with pytest.raises(HTTPException) as exc_info:
            annotation_service.remove_target(
                db=db_session, annotation_id=9999, target_id=1, user=test_user
            )
    ...
    def test_remove_target_permission_denied_for_non_creator(
        self, annotation_service, db_session, test_annotation, test_user, test_role_user
    ):
