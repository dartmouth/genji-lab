...
def create_document(document: DocumentCreate, db: AsyncSession = Depends(get_db)):
    ...
    collection = db.execute(
        select(DocumentCollection).filter(
            DocumentCollection.id == document.document_collection_id
        )
    ).scalar_one_or_none()
...
def update_document(
    ...
    if document.title:
        collection_id = (
            document.document_collection_id or db_document.document_collection_id
        )
        existing_document = db.execute(
            select(DocumentModel).filter(
...
def bulk_delete_documents(
    ...
    if missing_ids:
        raise HTTPException(
            status_code=404, detail=f"Documents not found: {list(missing_ids)}"
        )
    ...
    db.execute(delete(DocumentModel).where(DocumentModel.id.in_(request.document_ids)))
...
@router.get(
    "/collection/{collection_id}/with-stats", response_model=List[Dict[str, Any]]
)
        ...
        scholarly_count = db.execute(
            ...
            .filter(
                ...
                AnnotationModel.motivation.in_(
                    ["scholarly", "highlighting", "bookmarking", "classifying"]
                ),
            )
        ).scalar_one()
...
def import_word_document(
    ...
    collection = db.execute(
        select(DocumentCollection).filter(
            DocumentCollection.id == document_collection_id
        )
    ).scalar_one_or_none()
