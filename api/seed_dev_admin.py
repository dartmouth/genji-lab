...
def seed_site_settings(db: Session, admin_user: models.User):
    """Create site settings for development."""

    try:
        # Check if site settings already exist
        existing_settings = db.query(models.SiteSettings).first()
        ...
        print(f"✅ Site settings created successfully!")
...
def seed_admin_user(db: Session):
    ...
    try:
        ...
        user_password = models.UserPassword(
            user_id=admin_user.id, hashed_password=hashed_password
        )
...
def seed_document_collection(db: Session, admin_user: models.User):
    ...
    try:
        ...
        existing_collection = (
            ...
            .filter(models.DocumentCollection.title == "Sample Collection")
            .first()
        )
    ...
    try:
        with open(sample_doc_path, "rb") as f:
            ...
            if response.status_code == 201:
                ...
                print(
                    f"   Elements created: {result['import_results']['elements_created']}"
                )
            else:
...
def seed_classrooms(db: Session, admin_user: models.User):
    """Create test classrooms with date ranges."""
    from datetime import date, timedelta

    try:
        existing = db.query(models.Group).first()
        if existing:
            print("ℹ️  Classrooms already exist")
            return
        ...
        classrooms_data = [
            {
                "name": "Fall 2024 - Japanese Literature",
                "description": "Study of The Tale of Genji",
                "start_date": today - timedelta(days=60),  # Started 2 months ago
                "end_date": today + timedelta(days=30),  # Ends in 1 month
            },
            {
                "name": "Spring 2025 - Comparative Literature",
                "description": "Comparing translations across cultures",
                "start_date": today + timedelta(days=10),  # Starts soon
                "end_date": today + timedelta(days=120),  # 4 month course
            },
            {
                "name": "Summer 2024 - Advanced Studies",
                "description": "Deep dive into Heian period literature",
                "start_date": today - timedelta(days=90),  # Started 3 months ago
                "end_date": today - timedelta(days=10),  # Already ended
            },
        ]

        created_classrooms = []
        for data in classrooms_data:
            classroom = models.Group(
                name=data["name"],
        ...
        db.rollback()
        print(f"❌ Error creating classrooms: {e}")
        raise
    ...
    try:
        ...
        for user_data in test_users_data:
            ...
            role = (
                ...
                .filter(models.Role.name == user_data["role"])
                .first()
            )
            ...
            if not role:
                role = models.Role(
                    name=user_data["role"],
                    description=f"{user_data['role'].title()} role",
                )
                db.add(role)
                db.flush()
...
def assign_users_to_classrooms(db: Session, classrooms, users):
    """Assign test users to classrooms."""

    try:
        # Classroom 1: Fall 2024
        # instructor1 + students 1, 2, 3
        classrooms[0].members.extend([users[0], users[2], users[3], users[4]])

        # Classroom 2: Spring 2025
        # instructor2 + students 3, 4, 5
        classrooms[1].members.extend([users[1], users[4], users[5], users[6]])

        # Classroom 3: Summer 2024 (ended)
        # instructor1 + students 1, 4
        classrooms[2].members.extend([users[0], users[2], users[5]])

        db.commit()
        print("✅ Assigned users to classrooms")

    except Exception as e:
        db.rollback()
        print(f"❌ Error assigning users to classrooms: {e}")
        raise
...
def main():
    ...
    try:
        ...
        seed_site_settings(db, admin_user)
        collection = seed_document_collection(db, admin_user)

        # NEW: Seed classrooms and users
        classrooms = seed_classrooms(db, admin_user)
        users = seed_test_users(db)
        assign_users_to_classrooms(db, classrooms, users)
